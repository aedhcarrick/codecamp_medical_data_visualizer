...
def draw_cat_plot():
    ...
    df_cat = pd.melt(df, id_vars=['cardio'], value_vars=['cholesterol','gluc','smoke','alco','active','overweight']).groupby('cardio').value_counts().to_frame()
    df_cat.rename(columns={0:'total'}, inplace=True)

    # Draw the catplot with 'sns.catplot()'
    plot = sns.catplot(x='variable', y='total', hue='value', col='cardio', data=df_cat, 
